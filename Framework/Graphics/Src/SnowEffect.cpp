...
void SnowEffect::Initialize()
{
	...
	mSnowTextureID = TextureManager::Get()->LoadTexture("terrain/Snow.jpg");
...
void SnowEffect::Begin()
{
	...
	TextureManager::Get()->BindPS(mSnowTextureID, 2);
...
void SnowEffect::DebugUI()
{
	if (ImGui::CollapsingHeader("Snow Effect", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::DragFloat("Bump Offset", &mTessData.bumpOffset, 0.001f, -5.0f, 5.0f);
